    ...
    def _build(self, obj, stream, context):
        if self.include_name:
            ...
            for sc in self.subcons:
                ...
                context2 = context.__copy__()
                try:
                    sc._build(obj, stream2, context2) # nosec B110
                except Exception as e:
                    logger.debug("A subconstruct build threw. May or may not be an error: %s", e)
                    pass
                else:
                    context.__update__(context2)
...
class Buffered(Subconstruct):
    ...
    * resizer - a function that takes the size of the subcon and "adjusts"
    ...
    def _build(self, obj, stream, context):
        ...
        if not len(data) == size: raise AssertionError()
...
class Pass(Construct):
    ...
    * this construct is a singleton. do not try to instatiate it, as it
    ...
    def _parse(self, stream, context):
        pass
    def _build(self, obj, stream, context):
        if not obj is None: raise AssertionError()
    def _sizeof(self, context):
        return 0
    def __reduce__(self):
