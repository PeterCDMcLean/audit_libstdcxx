...
class CallFrameInfo(object):
    ...
    def _parse_cie_augmentation(self, header, entry_structs):
        ...
        if not augmentation.startswith(b'z'): raise AssertionError(
    ...
    def _parse_lsda_pointer(self, structs, stream_offset, encoding):
        ...
        """
        if not encoding != DW_EH_encoding_flags['DW_EH_PE_omit']: raise AssertionError()
        ...
        if modifier == DW_EH_encoding_flags['DW_EH_PE_absptr']:
            ...
            raise AssertionError('Unsupported encoding modifier for LSDA pointer: {:#x}'.format(modifier))
    ...
    def _parse_fde_header(self, entry_structs, offset):
        ...
        if encoding_modifier == 0:
            ...
            raise AssertionError('Unsupported encoding: {:#x}'.format(encoding))
