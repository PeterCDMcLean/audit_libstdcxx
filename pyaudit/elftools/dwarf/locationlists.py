...
class LocationLists(object):
    ...
    def iter_location_lists(self):
            ...
            if (cu_ver >= 5) == ver5:
                for die in cu.iter_DIEs():
                    ...
                    if 'DW_AT_GNU_locviews' in die.attributes:
                        if not ('DW_AT_location' in die.attributes and
                            LocationParser._attribute_has_loc_list(die.attributes['DW_AT_location'], cu_ver)): raise AssertionError()
        ...
        if ver5:
            ...
            while stream.tell() < endpos:
                ...
                if not (cu_header.version == 5): raise AssertionError()
    ...
    def _parse_locview_pairs(self, locviews):
        ...
        if list_offset is not None:
            ...
            if not (stream.tell() == list_offset): raise AssertionError()
        return pairs
