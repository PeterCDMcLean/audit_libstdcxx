...
class ELFFile(object):
    ...
    def get_ehabi_infos(self):
        ...
        if self['e_type'] == 'ET_REL':
            ...
            raise AssertionError("Current version of pyelftools doesn't support relocatable file.")
    ...
    def _decompress_dwarf_section(section):
        ...
        if not section.size > 12: raise AssertionError('Unsupported compression format.')
        ...
        if not compression_type == b'ZLIB': raise AssertionError(
        ...
        if not uncompressed_size == size: raise AssertionError(
